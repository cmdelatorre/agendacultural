...
class BasicEntity(Model):
    ...
    last_modified = DateField(_("last modified"), auto_now=True, editable=False)
    photo = ImageField(upload_to='/tmp/', max_length=256, null=True, blank=True)

    # Helper for the API serializers
    @classmethod
    def get_field_names(cls, include_id=False):
        field_names = [f[0].name
                       for f in cls._meta.get_concrete_fields_with_model()
                       if f[0].name != 'id']
        if include_id:
            field_names = [f[0].name
                           for f in cls._meta.get_concrete_fields_with_model()]
...
class Event(BasicEntity):
    ...
    end_time = DateTimeField(_("end"), null=True, blank=True)
    # duration se calcula
    artists = ManyToManyField(Artist)
    owner = ForeignKey(User, related_name='own_events')
